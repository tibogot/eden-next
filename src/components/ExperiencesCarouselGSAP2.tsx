    ...
    <section className="flex min-h-[400px] w-full flex-col bg-[#FAF3EB] px-4 py-10 md:flex-row md:px-8 md:py-20">
      ...
      <div className="flex w-full flex-col justify-between text-black md:w-1/3">
        <div>
          ...
          <p className="font-NHD mt-4 text-lg leading-tight text-gray-700">
      ...
      <div className="relative ml-10 overflow-hidden md:w-2/3">
        ...
        >
          {slides.map((img, i) => (
            <div
              key={i}
              className="flex h-[600px] w-[500px] shrink-0 items-center justify-center overflow-hidden bg-gray-200 text-4xl font-bold text-white"
            >
              <img
                src={img}
                alt={`Slide ${i % total}`}
                ...
                draggable={false}
              />
              <span className="absolute top-4 left-4 rounded bg-black/60 px-2 py-1 text-lg text-white">
                {i % total}
              </span>
            </div>
          ))}
