...
const Menu = () => {
  ...
  return (
    <div className="relative" ref={container}>
      {/* Header */}
      <div className="fixed top-0 left-0 w-full p-4 flex justify-between items-center z-40 bg-white/80 backdrop-blur-sm border-b border-gray-200">
        ...
        <div className="hidden md:flex items-center space-x-8">
          {menuLinks.map((link) => (
            <Link
              key={link.path}
              href={link.path}
              className="text-gray-700 hover:text-green-600 transition-colors duration-200 font-medium font-NHD"
            >
              {link.label}
            </Link>
          ))}
          <button className="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 transition-colors duration-200 font-medium font-NHD">
            Visit Us
          </button>
        </div>
        ...
        {/* Mobile menu button */}
        <button
          className="md:hidden cursor-pointer px-4 py-2 rounded-lg hover:bg-gray-100 transition-colors duration-200"
          onClick={toggleMenu}
      ...
      {/* Mobile Menu Overlay */}
      <div
        className="menu-overlay h-screen fixed top-0 left-0 w-full z-50 bg-lime-900 md:hidden"
        style={{
          clipPath: "polygon(0% 0%, 100% 0%, 100% 0%, 0% 0%)",
        }}
        ref={overlayRef}
